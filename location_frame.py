...
class LocAll:
    ...
    def init_para(self):
        self.methods = ['loc_seq6s', 'loc_geiger6s',
                        'loc_seq4s', 'loc_geiger4s']

    def init_gui(self):
        self.bar2 = ttk.LabelFrame(
            self.root, text='Figures Panel', width=1540, height=990, relief='sunken', borderwidth=1)
                ...
                if '/wnew_table' in h5file:
                    ...
                    if nt.nrows:
                        chose_data = {k2: [nt.cols._f_col(
                            '{}/x'.format(k2))[:], nt.cols._f_col('{}/y'.format(k2))[:]] for k2 in self.methods}
                        return chose_data
                    else:
