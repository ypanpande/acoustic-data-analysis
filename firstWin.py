...
class FirstWin:
    ...
    def init_gui(self):
        ...
        locmenu.add_command(label = 'Sequential6s', command = self.loc_sequential6s)
        ...
        self.cal1 = ttk.Label(self.root, text = 'year-month-day', background = "white", state = 'disabled')
        self.cal1.place(x = 290, y = 260, width = 100, height = 30)

        self.calchoose2 = ttk.Button(self.root, text = 'choose end date', command = self.choose_end_date, state = 'disabled')
...
# =============================================================================
        ...
        try:
            with tb.open_file(self.default_source_file, 'r') as h5file:
                for k, v in enumerate(h5file.walk_nodes('/', 'Table')): 
                    h5info.insert('{}.0'.format(k+1), 'Name:  ' + v.name+'        '+'Rows:  '+ str(v.nrows) +'\n')
        except IOError:
    ...
    def get_first_date(self):
        self.first_date = '{}-{:02}-{:02}'.format(self.beginDay['year_selected'],self.beginDay['month_selected'],self.beginDay['day_selected'])
    ...
    def get_end_date(self):
        self.end_date = '{}-{:02}-{:02}'.format(self.endDay['year_selected'],self.endDay['month_selected'],self.endDay['day_selected'])
        if datetime.datetime.strptime(self.end_date, '%Y-%m-%d') < datetime.datetime.strptime(self.first_date, '%Y-%m-%d'):
            messagebox.showerror(title = 'date info', message = 'end date should greater than begin date')
        else:
    ...
    def get_emerge_table(self, a, b):
        with tb.open_file(self.default_source_file, 'a') as ff:
            ...
            self.new_table = ff.create_table('/', 'wnew_table', ptf.Resultnested)
    ...
    def get_data(self):
        if self.dayVar.get() == 1:
            t1 = 't{}{:02}{:02}'.format(self.beginDay['year_selected'],self.beginDay['month_selected'],self.beginDay['day_selected'])
            if t1 in self.table_list:
                ...
                messagebox.showinfo(title = 'data info', message = 'there is no data am {}'.format(self.first_date))
        elif self.dayVar.get() == 2:
            t1 = 't{}{:02}{:02}'.format(self.beginDay['year_selected'],self.beginDay['month_selected'],self.beginDay['day_selected'])
            t2 = 't{}{:02}{:02}'.format(self.endDay['year_selected'],self.endDay['month_selected'],self.endDay['day_selected'])
            ...
            if t1_idx >= (len(self.table_list) - 1):
                ...
                messagebox.showerror(title = 'data info', message = 'Selected date is out of date range in the data file')
            else:
...
# =============================================================================
    ...
    def loc_sequential6s(self):
        ...
        date = '{}-{:02}-{:02}'.format(self.beginDay['year_selected'],self.beginDay['month_selected'],self.beginDay['day_selected'])
