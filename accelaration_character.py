...
from time_field_character import TimeField
...
#            ttk.Checkbutton(self.bar1, text = k, variable = self.CH[k], onvalue = 1, offvalue = 0, state = 'unchecked').place(x = 115 + 100*i, y = 5, width = 95, height = 25)
        ...
        for m, n in enumerate(sorted(self.timef.keys())):
            ttk.Checkbutton(self.bar1, text=n, variable=self.timef[n], onvalue=1, offvalue=0,  state='unchecked').place(
                x=115 + 100*m, y=15, width=95, height=25)

        ttk.Label(self.bar1, text='Choose Function:').place(
            x=5, y=70, width=100, height=25)
        ...
        for k2 in self.figure_number()['para']:
            ...
            if self.cfunction.get() == self.Function[1]:
                ...
                aa.hist(d, bins=self.hbins.get(), color=self.hcolor.get()[
                        0], rwidth=float(self.hrwidth.get()))
                aa.set_title('{}'.format(k2))
        ...
        try:
            with tb.open_file(self.path, mode='r') as h5file:
                if '/wnew_table' in h5file:
                    ...
                    if nt.nrows:
                        chose_data = {k2: nt.cols._f_col(
                            '{}/{}'.format(self.field, k2))[:] for k2 in self.get_chose_para(self.timef)}
                        return chose_data
                    else:
...
class FirstCh(AccField):
    ...
    def init_para_data(self):
        ...
        self.timef = {'shorttime_Ch': self.firstch_var,
                      'shorttime_t': self.firstt_var}
        ...
        try:
            with tb.open_file(self.path, mode='r') as h5file:
                if '/wnew_table' in h5file:
                    nt = h5file.root.wnew_table
                    if nt.nrows:
                        chose_data = {k2: nt.cols._f_col('{}'.format(k2))[
                            :] for k2 in self.get_chose_para(self.timef)}
