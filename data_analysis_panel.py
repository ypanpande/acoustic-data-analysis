...
LOC_METHODS = ['--Choose Loc Method--', 'loc_seq6s',
               'loc_geiger6s', 'loc_seq4s', 'loc_geiger4s']
TIMEFIELD_PRAMETERS = ['--Timefield Not Selected--', 'maxAmp_t',
                       'time_peak', 'Energy', 'ZeroCrossf', 'rise_time', 'RA']
FREFIELD_PRAMETERS = ['--Frefield Not Selected--', 'maxAmp_f',
                      'fre_peak', 'fre_centroid', 'fre_wpeak', 'Power']
DAY = [str(i+1).zfill(2) for i in range(31)]
MONTH = [str(i+1).zfill(2) for i in range(12)]
YEAR = ['2018', '2019', '2020', '2021', '2022']
TIME_INTERVAL = ['--Choose Time Interval--', '1 minute', '5 minutes',
...
class graphpanel:
    ...
    def create_top_settings_bar(self):
        ...
        tk.Label(topbar_frame, text='Time Range:', font=("Helvetica", 11), bg='peach puff',
                 relief='ridge', borderwidth=1).grid(row=1, column=0, columnspan=2, padx=2, pady=5)
...
#                textvariable=self.start_time_month, command=self.start_time_month_changed).grid(row=4, column=2, rowspan = 2, padx = 3, pady = 5)
        combo_start_year = ttk.Combobox(
            topbar_frame, textvariable=self.start_time_year, value=YEAR, width=4, state='randonly')
        combo_start_year.grid(row=4, column=3, rowspan=2,
                              columnspan=2, padx=3, pady=5)
        combo_start_year.current(0)
        combo_start_year.bind('<<ComboboxSelected>>',
                              self.start_time_year_changed)
...
#                textvariable=self.start_time_year, command=self.start_time_year_changed).grid(row=4, column=3, rowspan = 2, columnspan=2, padx = 3, pady = 5)
        ...
        tk.Label(topbar_frame, text='To', font=("Helvetica", 10), fg='blue').grid(
            row=4, column=6, rowspan=2, padx=10, pady=5)

        combo_end_day = ttk.Combobox(
...
#                textvariable=self.end_time_month, command=self.end_time_month_changed).grid(row=4, column=8, rowspan = 2, padx = 3, pady = 5)
        ...
        combo_end_year = ttk.Combobox(
            topbar_frame, textvariable=self.end_time_year, value=YEAR, width=4, state='randonly')
        combo_end_year.grid(row=4, column=9, rowspan=2,
                            columnspan=2, padx=3, pady=5)
...
#                textvariable=self.end_time_year, command=self.end_time_year_changed).grid(row=4, column=9, rowspan = 2, columnspan=2, padx = 3, pady = 5)
        ...
        ttk.Separator(topbar_frame, orient='vertical').grid(
            row=0, column=11, rowspan=10, sticky='ns', padx=10)
        tk.Label(topbar_frame, text='Location Method:', font=("Helvetica", 11), bg='peach puff',
                 relief='ridge', borderwidth=1).grid(row=1, column=12, columnspan=3, padx=10, pady=5)

        combo1 = ttk.Combobox(topbar_frame, textvariable=self.location_method,
                              value=LOC_METHODS, width=22, state='randonly')
...
#                     sticky= 'n'+'w'+'e'+'s', padx=4, pady=4)
        ttk.Separator(topbar_frame, orient='vertical').grid(
            row=0, column=16, rowspan=10, sticky='ns', padx=20)
        tk.Label(topbar_frame, text='Select Location Range(mm):', font=("Helvetica", 11), bg='peach puff',
                 relief='ridge', borderwidth=1).grid(row=1, column=17, columnspan=5, padx=2, pady=5)
        tk.Label(topbar_frame, text='x1', font=("Helvetica", 10),
                 fg='blue').grid(row=4, column=17, padx=1, pady=1)
        x1_entry = tk.Entry(topbar_frame, textvariable=self.x1,
                            width=8, validatecommand=self.x1_changed)
        x1_entry.grid(row=4, column=18, padx=2, pady=2)
        x1_entry.bind('<Return>', self.x1_changed)

        tk.Label(topbar_frame, text='x2', font=("Helvetica", 10),
                 fg='blue').grid(row=5, column=17, padx=1, pady=5)
        ...
        tk.Label(topbar_frame, text='y1', font=("Helvetica", 10),
                 fg='blue').grid(row=4, column=19, padx=1, pady=1)
        ...
        tk.Label(topbar_frame, text='y2', font=("Helvetica", 10),
                 fg='blue').grid(row=5, column=19, padx=1, pady=5)
        ...
        confirm_button = ttk.Button(
            topbar_frame, text='Confirm', command=self.location_range_selected)
        confirm_button.grid(row=5, column=22, columnspan=2, padx=6, pady=5)

        ttk.Separator(topbar_frame, orient='vertical').grid(
            row=0, column=24, rowspan=10, sticky='ns', padx=10)
        tk.Label(topbar_frame, text='Channel:', font=("Helvetica", 11), bg='peach puff',
                 relief='ridge', borderwidth=1).grid(row=1, column=25, columnspan=3, padx=10, pady=5)

        combo_ch = ttk.Combobox(
            topbar_frame, textvariable=self.channel, value=CH, width=22, state='randonly')
        combo_ch.grid(row=4, column=25, rowspan=2,
                      columnspan=3, padx=10, pady=2)
        #self.combo1['value'] = LOC_METHODS
        combo_ch.current(0)
        combo_ch.bind('<<ComboboxSelected>>', self.set_channel)

    def create_heatmap_bar(self):
        heatmapbar_frame = tk.Frame(
            self.root, height=450, width=780, relief='ridge', borderwidth=1)
        heatmapbar_frame.grid(row=12, column=0, rowspan=10, columnspan=10,
                              sticky='n'+'w'+'e'+'s', padx=4, pady=4)
        tk.Label(heatmapbar_frame, text='Heatmap', font=(
            "Helvetica", 10), fg='blue').grid(row=12, column=0,  pady=1)
        ...
        self.heatmap_frame = tk.Frame(
            heatmapbar_frame, height=390, width=750, relief='groove', borderwidth=0.5)
        self.heatmap_frame.grid(row=13, column=0, rowspan=9, columnspan=10,
                                sticky='n'+'w'+'e'+'s', padx=4, pady=4)

    def create_hist_bar(self):
        histbar_frame = tk.Frame(
            self.root, height=450, width=780, relief='ridge', borderwidth=1)
        histbar_frame.grid(row=24, column=0, rowspan=10, columnspan=10,
                           sticky='n'+'w'+'e'+'s', padx=4, pady=4)
        tk.Label(histbar_frame, text='Histgramm (N vs time)', font=(
            "Helvetica", 10), fg='blue').grid(row=24, column=0, pady=1)
        ...
        combo_hist = ttk.Combobox(histbar_frame, textvariable=self.time_interval_var,
                                  value=TIME_INTERVAL, width=22, state='randonly')
        ...
        self.hist_frame = tk.Frame(
            histbar_frame, height=390, width=750, relief='groove', borderwidth=0.5)
        self.hist_frame.grid(row=25, column=0, rowspan=9, columnspan=10,
                             sticky='n'+'w'+'e'+'s', padx=4, pady=4)

    def create_timefield_bar(self):
        timefieldbar_frame = tk.Frame(
            self.root, height=450, width=780, relief='ridge', borderwidth=1)
        timefieldbar_frame.grid(row=12, column=15, rowspan=10, columnspan=10,
                                sticky='n'+'w'+'e'+'s', padx=4, pady=4)

        tk.Label(timefieldbar_frame, text='Hist time field', font=(
            "Helvetica", 10), fg='blue').grid(row=12, column=15,  pady=1)
        ...
        combo2 = ttk.Combobox(timefieldbar_frame, textvariable=self.timefield_var,
                              value=TIMEFIELD_PRAMETERS, width=25, state='randonly')
...
#        ttk.OptionMenu(timefieldbar_frame, self.timefield_var, *TIMEFIELD_PRAMETERS).grid(row = 12, column = 16, columnspan = 2, padx = 1, pady = 2)
        self.timefield_frame = tk.Frame(
            timefieldbar_frame, height=390, width=750, relief='groove', borderwidth=0.5)
        self.timefield_frame.grid(row=13, column=15, rowspan=9, columnspan=10,
                                  sticky='n'+'w'+'e'+'s', padx=4, pady=4)

    def create_frefield_bar(self):
        frefieldbar_frame = tk.Frame(
            self.root, height=450, width=780, relief='ridge', borderwidth=1)
        frefieldbar_frame.grid(row=24, column=15, rowspan=10, columnspan=10,
                               sticky='n'+'w'+'e'+'s', padx=4, pady=4)
        tk.Label(frefieldbar_frame, text='Hist frequency field', font=(
            "Helvetica", 10), fg='blue').grid(row=24, column=15,  pady=1)
        ...
        combo3 = ttk.Combobox(frefieldbar_frame, textvariable=self.frefield_var,
                              value=FREFIELD_PRAMETERS, width=25, state='randonly')
...
#        ttk.OptionMenu(frefieldbar_frame, self.frefield_var, *FREFIELD_PRAMETERS).grid(row = 24, column = 16, columnspan = 2, padx = 1, pady = 2)
        self.frefield_frame = tk.Frame(
            frefieldbar_frame, height=390, width=750, relief='groove', borderwidth=0.5)
...
#        return self.init_setting['heatmap']
    ...
    def setting_time_info(self, event=None):
        messagebox.showinfo(
            'setting time', 'start time is greater than end time')
...
#            self.y = y[~np.isnan(y)]
    ...
    def create_new_table(self):
        ...
        self.new_table = self.h5file.create_table(
            '/', 'new_table', Resultnested)

    def show_table_info(self, event=None):
        messagebox.showinfo('show_table_info',
                            'there is not location data selected')
...
############time histogram#############################################################
    ...
    def error_info_2(self, event=None):
        messagebox.showerror(
            'Error', 'value is not in the range [-2000, 1200]')
    ...
    def show_loc_range_info(self, event=None):
        messagebox.showinfo('select location range',
                            'please select the location range of interest')
        ...
        if '/new_table' in self.h5file:
            ...
            if num_row:
                ...
                for i in range(num_row):
                    temp = measD[i].decode(
                        'UTF-8') + ' ' + measH[i].decode('UTF-8').replace('.', ':')
                    measT.append(temp)
                return measT
    ...
    def show_time_interval_info(self):
        messagebox.showinfo('show_time_interval_info',
                            'the time interval is not selected')

    def show_meet_condition_info(self):
        messagebox.showinfo('show_meet_condition_info',
                            'there is no event in the selected location range')

    def show_new_table_info(self):
        messagebox.showinfo('show_table_info',
                            'there is no related data created')

    def show_new_table_time_info(self):
        messagebox.showinfo('show_table_time_info',
...
##########timefield figure#######################################################
    ...
    def show_timefield_info(self):
        messagebox.showinfo('show_timefield_info',
                            'timefield parameter is not selected')
    ...
    def create_timefield_hist_figure(self):
        ...
        a.set_title('Time field figure: {}'.format(
            self.get_timefield()), fontsize=12)
    ...
    def get_timefield_data(self):
        ...
        if '/new_table' in self.h5file:
            ...
            num_row = a.nrows
            if num_row:
                timefield_data = a.cols._f_col(
                    '{}/timefield/{}'.format(channel, timefield_parameter))
    ...
    def embedd_timefield_hist_figure(self):
        ...
        canvas = FigureCanvasTkAgg(
            self.af1, master=self.timefield_canvas_frame)
        canvas.draw()
        canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=1)
...
##########frefield figure####################################################################
    ...
    def show_frefield_info(self):
        messagebox.showinfo('show_frefield_info',
                            'frefield parameter is not selected')
    ...
    def create_frefield_hist_figure(self):
        ...
        a.set_title('Freq field figure: {}'.format(
            self.get_frefield()), fontsize=12)
        a.set_xlabel('Count', fontsize=10)
    ...
    def get_frefield_data(self):
        ...
        if '/new_table' in self.h5file:
            ...
            num_row = a.nrows
            if num_row:
                frefield_data = a.cols._f_col(
                    '{}/frefield/{}'.format(channel, frefield_parameter))
